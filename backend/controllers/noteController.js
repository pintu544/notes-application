...
const CreateNote = asyncHandler(async (req, res) => {
  ...
  if (!title || !content || !category) {
    ...
    io.to(req.user._id.toString()).emit("notes:create", createdNote);
...
const DeleteNote = asyncHandler(async (req, res) => {
  ...
  io.to(req.user._id.toString()).emit("notes:delete", req.params.id);
  if (note) {
    await note.remove();
...
const UpdateNote = asyncHandler(async (req, res) => {
  ...
  if (note) {
    ...
    io.to(req.user._id.toString()).emit("notes:update", updatedNote);
